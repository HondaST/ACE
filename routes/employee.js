    ...
    if (available === 'true') {
      where += ` AND NOT EXISTS (
        ...
        WHERE  i2.suie     = pe.suie
          AND  i2.void_ind = 'N'
          AND  i2.inv_date BETWEEN (SELECT season_start FROM season WHERE GETDATE() BETWEEN season_start AND season_end)
                               AND (SELECT season_end   FROM season WHERE GETDATE() BETWEEN season_start AND season_end)
      )`;
    }
